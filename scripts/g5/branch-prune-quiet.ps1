...
try {
  ...
  if ($ConfirmApply) {
    ...
    $outcome='PREVIEW'
    $msg="would delete: $($gone -join ', ')"
    Write-Host "`n[HINT] 적용하려면: `$env:CONFIRM_APPLY='true'; pwsh -File .\scripts\g5\branch-prune-quiet.ps1 -ConfirmApply"
  }

  :log_and_exit
  $rec=@{timestamp=(Get-Date).ToString('o');level='INFO';traceId=$trace;module='git';action='branch-prune-quiet';inputHash='';outcome=$outcome;durationMs=$sw.ElapsedMilliseconds;errorCode='';message=$msg} | ConvertTo-Json -Compress
