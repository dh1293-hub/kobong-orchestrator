...
$SrvDir = Join-Path $Repo 'containers\orch-shells'
...
function Test-PortUsed([int]$Port){
  (Get-NetTCPConnection -ErrorAction SilentlyContinue | ? LocalPort -eq $Port | ? State -in 'Listen','Established').Count -gt 0
}
...
function Stop-ByPort([int]$Port){
  ...
  foreach($procId in $pids){
    try{ Stop-Process -Id $procId -Force -ErrorAction Stop; Write-Host "[CLEAN] killed PID=$procId on :$Port" -ForegroundColor Yellow }
    catch{  Write-Host "[CLEAN] kill fail PID=$procId on :$Port -> $($_.Exception.Message)" -ForegroundColor Red }
  }
}
...
function Wait-HttpOK($Url, [int]$TimeoutSec=60){
  $t0=Get-Date
  do{
    try{ $r=Invoke-WebRequest -UseBasicParsing -Uri $Url -TimeoutSec 5; if($r.StatusCode -ge 200 -and $r.StatusCode -lt 300){ return $true } }catch{}
...
function Start-NodeServer([int]$Port,[string]$Mode){
  if(!(Test-Path $SrvJs)){ throw "server.js not found: $SrvJs" }
  $node = Get-NodePath
  if(!$node){ throw "Node.exe not found. Install Node 18+ or adjust path in script." }
...
function Open-UI([int]$MockPort,[int]$DevPort){
  # 헬스 우선 확인 (MOCK 기준)
  if(Wait-HttpOK "http://localhost:$MockPort/health" 30){
