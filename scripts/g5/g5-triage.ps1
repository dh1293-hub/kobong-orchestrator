...
foreach($r in $recs){
  ...
  $codeRaw = FirstProp $r @('error','errorCode','category','code'); if (-not $codeRaw) { $codeRaw = $isErr ? 'ERROR' : '' }
  $msgRaw = FirstProp $r @('message','msg','detail','errorMessage','exception')
  if (-not $isErr) { continue } # 에러/실패만 버킷팅
  ...
  $errBuckets[$key].n = [int]$errBuckets[$key].n + 1
  if ($msgRaw) { $errBuckets[$key].sample = $msgRaw }
}

$top = $errBuckets.Values | Sort-Object @{Expression={ [int]($_.n) }; Descending=$true} | Select-Object -First $Top
...
if ($OneLine) {
  $arr=@(); foreach($t in $top){ $arr += ("{0} x{1}" -f $t.code,[int]$t.n) }
...
if ($top.Count -gt 0) {
  ...
  foreach($t in $top){ Write-Host (" - [{0}] x{1} :: {2}" -f $t.code,[int]$t.n,(Short $t.sample 140)) -ForegroundColor DarkYellow }
} else {
