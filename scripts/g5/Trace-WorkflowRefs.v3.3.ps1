...
- 상대경로 해석: "YAML 폴더 우선 → 레포 루트 → scripts/ → scripts/g5/ → .github/scripts/".
- 파일명만 토큰(예: ak-rewrite.ps1)도 위 우선순위로 해석.
- '.github/workflows/inventory-ci.yml' 은 지정 제외.
- -ScanDepth 단계만큼 반복 루프(전체 검색) 확장. 출력은 항상 평탄화된 파일 목록.
...
# GitHub Actions 내부에서는 자동으로 $GITHUB_WORKSPACE 를 루트로 사용.

[산출물]
- _inventory/workflows.txt  (UTF-8, 덮어쓰기)
#>
...
function Strip-LineComments([string]$text){
  # 라인 "시작"이 '#'인 주석만 제거(인라인 #은 보존) — 규칙 고정
...
function Resolve-Candidate([string]$token,[string]$baseFile,[string]$repoRoot){
  ...
  if([IO.Path]::IsPathRooted($tok)){
    ...
    $tok2 = $tok -replace '\\','/'
    $candidates = @(
      (Join-Path $yamlDir  $tok2),
      (Join-Path $repoRoot $tok2)
    )
  } else {
    # 파일명만 있을 때: 우선 디렉터리 탐색 → 마지막 보루: scripts/ 트리 재귀
    ...
    if(Test-Path $scriptsRoot -PathType Container){
      $hit = Get-ChildItem -Path $scriptsRoot -Recurse -Filter $tok -File -ErrorAction SilentlyContinue | Select-Object -First 1
  ...
  foreach($c in ($candidates | Select-Object -Unique)){
    $abs = (Resolve-Path $c -ErrorAction SilentlyContinue)?.Path
    if($abs){
      try{
        $it = Get-Item $abs -ErrorAction Stop
        if(-not $it.PSIsContainer){ return $it.FullName } # 파일만
      }catch{}
    }
  }
  return $null
...
$WfDir    = Join-Path $RepoRoot '.github/workflows'   # 슬래시(크로스 OS)
...
$WfFiles = @(
  ...
  # ⛔ inventory-ci.yml 제외(대소문자 무시)
  $_.BaseName -ne 'inventory-ci'
} | Sort-Object FullName -Unique

$Lines = New-Object System.Collections.Generic.List[string]
$stamp = Get-Date -Format 'yyyy-MM-dd HH:mm:ssK'
$Lines.Add("# GENERATED @ $stamp  (repo=$RepoRoot)")
$Lines.Add("# RULES: comments-excluded, files-only, .github/workflows only, flat paths, exclude=inventory-ci.yml")
$Lines.Add("")
...
foreach($wf in $WfFiles){
  ...
  while($curDepth -lt $ScanDepth -and $scanQueue.Count -gt 0){
    ...
    while($scanQueue.Count -gt 0){
      ...
      try{
        $fileRaw = Get-Content -LiteralPath $absPath -Raw -ErrorAction Stop
      }catch{ continue }
