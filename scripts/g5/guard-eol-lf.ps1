...
try {
  ...
  foreach ($f in $toLF) {
    ...
    if ($rxCRLF.IsMatch($raw)) { $plan += [pscustomobject]@{ Path=$f.FullName; Mode='LF';   Reason='CRLF→LF' } }
  }
  foreach ($f in $toCRLF) {
    $raw = Get-Content -Raw -Encoding utf8 $f.FullName
    if ($rxLF.IsMatch($raw))   { $plan += [pscustomobject]@{ Path=$f.FullName; Mode='CRLF'; Reason='LF→CRLF for .cmd' } }
  }

  # .gitattributes 확인/보강 (라인 비교 + 필요한 규칙만 append)
  ...
  $needAttr = $false; $missingLines = @(); $existingAttr = Test-Path $AttrPath
  if (-not $existingAttr) { $needAttr = $true; $missingLines = $attrWanted }
  else {
    $curLines = Get-Content -Encoding utf8 $AttrPath | ForEach-Object { $_.TrimEnd() }
    foreach ($ln in $attrWanted) { if (-not ($curLines -contains $ln)) { $missingLines += $ln } }
  ...
  if (-not $ConfirmApply) {
    ...
    Write-Host "적용하려면:`n  `$env:CONFIRM_APPLY='true'; pwsh -File scripts/g5/guard-eol-lf.ps1 -ConfirmApply"
  ...
  foreach ($it in $plan) {
    if ($it.Mode -eq 'ATTR') {
      ...
      if ($wasExisting) {
        $curLines = Get-Content -Encoding utf8 $AttrPath | ForEach-Object { $_.TrimEnd() }
        $newAttr  = ($curLines + $it.Missing) -join "`n"; $newAttr += "`n"
        ...
        Write-Host ("[fixed] .gitattributes — appended ({0}) — backup: {1}" -f ($it.Missing -join ', '), $bak)
      } else {
        ...
        Write-Host "[fixed] .gitattributes — new file"
      }
      continue
    }
    ...
    elseif ($it.Mode -eq 'CRLF') { $new = ($src -replace "`r?`n","`r`n") }
  ...
  Write-Log 'INFO' 'summary' 'EOL normalization done' 'OK'
}
catch {
