...
try {
  ...
  foreach ($f in $files) {
    ...
    if ($missing.Count -gt 0) { $plan += [pscustomobject]@{ Path=$f.FullName; Missing=$missing -join ', ' } }
  }

  if (-not $plan) {
    Write-Host "✅ 모든 PowerShell 스크립트가 PS7 안전 헤더를 갖추었습니다."
    Write-Log 'INFO' 'scan' 'all good' 'OK'
    return
  }

  Write-Host ("📝 헤더 누락 파일 {0}개" -f $plan.Count)
  $plan | ForEach-Object { Write-Host (" - {0} :: {1}" -f $_.Path, $_.Missing) }

  if (-not $ConfirmApply) {
    Write-Host "`nDRY-RUN ✅ 실제 수정은 하지 않았습니다."
    Write-Host "적용하려면:`n  `$env:CONFIRM_APPLY='true'; pwsh -File scripts/g5/guard-ps7-header.ps1 -ConfirmApply"
  ...
  foreach ($it in $plan) {
    ...
    Write-Log 'INFO' 'apply' "inserted header → $p" 'APPLIED' '' ((Get-FileHash $p -Algorithm SHA256).Hash)
    Write-Host "[fixed] $p — backup: $bak"
  }

  Write-Host "`nDONE ✅ 헤더 삽입 완료"
  Write-Log 'INFO' 'summary' 'PS7 header enforced' 'OK'
}
catch {
