...
const server = http.createServer(async (req,res)=>{
  ...
  try{
    ...
    if (req.method==="GET" && p==="/api/ghmon/repos"){
      ...
      const list = await listReposByOwner(owner);
    ...
    if (req.method==="GET" && p==="/api/ghmon/pr/checks"){
      const no = Number(u.query.number||0);
      if (!no) return bad(res, 400, "number required", common);
      const data = await getPRChecks(no);
      return ok(res, { ...common, number:no, ...data });
    }
    if (req.method==="GET" && p==="/api/ghmon/pr/timeline"){
      const no = Number(u.query.number||0);
      if (!no) return bad(res, 400, "number required", common);
      const items = await getPRTimeline(no);
      return ok(res, { ...common, number:no, items });
    }
    if (req.method==="GET" && p==="/api/ghmon/pr/files"){
      const no = Number(u.query.number||0);
      if (!no) return bad(res, 400, "number required", common);
      const items = await getPRFiles(no);
      return ok(res, { ...common, number:no, items });
    }
// ---- SECURITY 라우트(이 카드 전용) ----
// GET /api/ghmon/security/code
if (req.method==="GET" && p==="/api/ghmon/security/code"){
  const items = await listCodeAlerts(u.query||{});
  return ok(res, { ...common, items });
}
// GET /api/ghmon/security/deps  (Dependabot: 기존 /alerts 별칭)
if (req.method==="GET" && p==="/api/ghmon/security/deps"){
  const items = await listAlerts(u.query||{});
  return ok(res, { ...common, items });
}
// GET /api/ghmon/security/secret
if (req.method==="GET" && p==="/api/ghmon/security/secret"){
  const items = await listSecretAlerts(u.query||{});
  return ok(res, { ...common, items });
}
