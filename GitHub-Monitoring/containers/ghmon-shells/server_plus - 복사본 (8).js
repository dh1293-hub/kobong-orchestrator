...
const { spawn } = require("child_process"); // ★ Shell Ops용
...
const server = http.createServer(async (req,res)=>{
  ...
  try{
    ...
    if (req.method==="POST" && p==="/api/ghmon/release/create"){
      const q = u.query || {};
      // JSON 바디 파싱
      let body = {}; await new Promise(r => { const b=[]; req.on("data",c=>b.push(c)); req.on("end",()=>{ try{ body=JSON.parse(Buffer.concat(b).toString("utf8")||"{}"); }catch(_){ body={}; } r(); }); });
      const tag    = String(body.tag || q.tag || "").trim();       // 예: v1.2.3
      ...
      return ok(res, { ...common, release: { id: rel.id, html_url: rel.html_url, tag_name: rel.tag_name } });
    }

    // “배포 트리거”: repository_dispatch 일반화
    if (req.method==="POST" && p==="/api/ghmon/release/trigger-deploy"){
      ...
      const r = await ghWrite(`/repos/${owner}/${repo}/dispatches`, "POST", { event_type, client_payload: payload });
      return ok(res, { ...common, dispatched: true, event_type, payload });
    }

    // “백포트 생성”: 추적 이슈를 만들어 파이프라인/담당자에게 넘김(가볍고 안전)
    if (req.method==="POST" && p==="/api/ghmon/release/backport"){
      let body = {}; await new Promise(r => { const b=[]; req.on("data",c=>b.push(c)); req.on("end",()=>{ try{ body=JSON.parse(Buffer.concat(b).toString("utf8")||"{}"); }catch(_){ body={}; } r(); }); });
      const tag    = String(body.tag || "").trim();
      const target = String(body.target || "release").trim(); // 예: release/1.0 또는 release
      if (!tag) return bad(res, 400, "tag required", common);
