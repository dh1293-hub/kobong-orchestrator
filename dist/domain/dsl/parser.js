...
function parseSource(val) {
    ...
    if (m)
        return { type: m[1].toLowerCase(), path: m[2].trim() };
...
function parseFormat(val) {
    ...
    if (m)
        return { type: m[1].toLowerCase() };
    const m2 = val.match(/^(type)\s*[:= ]\s*([A-Za-z][\w-]*)$/i);
    if (m2)
        return { type: m2[2].toLowerCase() };
    return null;
}
export function parseFromDsl(input) {
    if (typeof input !== "string")
        throw new ParseError("input must be a string DSL", 0);
    ...
    for (const line of lines) {
        ...
        if (!kv)
            err(ctx, "invalid syntax (expected 'key: value' / 'key=value' / 'key value')");
        switch (kv.key) {
            case "version":
            ...
            case "source": {
                ...
                if (!src)
                    err(ctx, "invalid source (e.g., 'source: csv tests/fixtures/minimal.csv')");
                spec.source = src;
                break;
            }
            case "fields": {
                ...
                if (!fs)
                    err(ctx, "invalid fields (comma/space separated)");
                spec.fields = fs;
                break;
            }
            case "format": {
                ...
                if (!fm)
                    err(ctx, "invalid format (e.g., 'format: table')");
                spec.format = fm;
                break;
            }
            default:
    ...
    if (!spec.source)
        throw new ParseError("source 미지정", ctx.lineNo || 0);
    if (!spec.fields || spec.fields.length === 0)
        throw new ParseError("fields 미지정", ctx.lineNo || 0);
    if (!spec.format)
        throw new ParseError("format 미지정", ctx.lineNo || 0);
    return spec;
}
