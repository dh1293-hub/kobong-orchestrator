...
import { Card } from "@/components/kui/Card";
import { KPI } from "@/components/kui/KPI";
import { Sparkline } from "@/components/kui/Sparkline";
import { TinyBars } from "@/components/kui/TinyBars";
import { IcPR, IcIssue, IcRun, IcClock, IcAlert, IcBranch, IcStar, IcBolt } from "@/components/kui/Icon";
...
export default function KobongGitHubOps360() {
  ...
  React.useEffect(()=> {
    ...
    document.body.classList.add("bg-[hsl(var(--background))]","text-[hsl(var(--foreground))]");
  },[]);
  ...
  return (
    <div className="min-h-screen w-full bg-[hsl(var(--background))] text-[hsl(var(--foreground))]">
      ...
      <div className="sticky top-0 z-40 backdrop-blur-md bg-black/30 border-b border-white/10">
        <div className="mx-auto max-w-[1800px] px-5 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="size-8 rounded-lg bg-gradient-to-br from-emerald-400/70 to-cyan-400/70 glow"/>
            <div>
          ...
          <div className="flex items-center gap-2">
            <input className="glass text-sm rounded-lg px-3 py-1.5 w-[220px] placeholder:text-white/50" placeholder="Quick filter (repo / branch / user)" />
      ...
      <div className="mx-auto max-w-[1800px] px-5 py-5 grid grid-cols-12 gap-4 auto-rows-[minmax(90px,auto)]">
        ...
        <Card title="Active Issues" subtitle="Severity · age · labels" className="col-span-12 lg:col-span-5">
          <div className="space-y-2 text-sm">
            {data.issues.map((i:Issue)=>(
              <div key={i.id} className="flex items-center justify-between glass rounded-lg px-3 py-2">
                <div className="flex items-center gap-2">
                  <IcIssue/>
                  <div>
        ...
        <Card title="Release Cadence" subtitle="last 12 months" className="col-span-12 lg:col-span-7">
          <div className="grid grid-cols-12 gap-2">
            {new Array(12).fill(0).map((_,i)=> {
              ...
              return <div key={i} className="h-16 glass rounded-md flex items-end p-2">
        ...
        {/* Footer density filler: avoid empty spaces */}
        <div className="col-span-12 grid grid-cols-1 md:grid-cols-3 gap-3">
          <div className="glass rounded-xl p-4">
            <div className="text-xs text-white/60 mb-2 flex items-center gap-2"><IcBranch/> Branch Activity</div>
