...
const VERSION = "orchmon-mock v2.1 (line-mode+cp949)";
...
const srv = http.createServer((req,res)=>{
  ...
  if(req.method==="GET" && p==="/health") return j(res,200,{ ok:true, service:"orchmon", mode:"MOCK", uptimeSec:process.uptime(), ts:new Date().toISOString(), version:VERSION });
...
function openSession(role){
  ...
  if(shell.toLowerCase().includes("powershell")){
    ...
    args.push("-NoProfile");
    args.push("-Command");
    args.push("[Console]::InputEncoding=[Text.Encoding]::UTF8; [Console]::OutputEncoding=[Text.Encoding]::UTF8; $Host.UI.RawUI.WindowTitle='orchmon'; $PSStyle.OutputRendering='Ansi'; powershell");
  }
  if(PTY){
    try{
...
srv.listen(PORT, ()=> console.log(`[ORCHMON v2.1] http://localhost:${PORT} — WS ${WS_PATH}`));